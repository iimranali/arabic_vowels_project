...
RUN_DIR = PROJECT_ROOT / "runs_torch" / "safe_mtl_efficientnetb0_direct84_letter_vowel_makhraj"
...
class EfficientNetB0MTLDirect84LetterVowelMakhraj(nn.Module):
    def __init__(self):
        super().__init__()

        model = efficientnet_b0(weights=None)

        # EfficientNet-B0 first conv expects 3-channel RGB.
        # Replace it with 1-channel input for log-Mel spectrogram.
    ...
    def forward(self, x):
        ...
        return {
            "direct84": self.direct84_head(features),
            "letter": self.letter_head(features),
            "vowel": self.vowel_head(features),
            "makhraj": self.makhraj_head(features),
        }
...
def plot_training_curves(history_df: pd.DataFrame):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("EfficientNet-B0 MTL Direct84+Auxiliary Loss")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("EfficientNet-B0 MTL Direct84+Auxiliary Accuracy")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def main():
    ...
    model = EfficientNetB0MTLDirect84LetterVowelMakhraj().to(DEVICE)
    ...
    summary = {
        "model": "mtl_efficientnetb0_direct84_letter_vowel_makhraj",
        "seed": SEED,
