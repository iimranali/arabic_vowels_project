...
RUN_DIR = PROJECT_ROOT / "runs_torch" / "safe_mtl_resnet18_letter_vowel_makhraj"
...
class ResNet18MTLLetterVowelMakhraj(nn.Module):
    def __init__(self):
        super().__init__()
...
def compute_batch_predictions(outputs):
    ...
    return letter_pred, vowel_pred, makhraj_pred, derived84_pred
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        letter_pred, vowel_pred, makhraj_pred, derived84_pred = compute_batch_predictions(outputs)
...
def plot_training_curves(history_df: pd.DataFrame):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("MTL ResNet18 Letter+Vowel+Makhraj Loss")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("MTL ResNet18 Letter+Vowel+Makhraj Accuracy")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def main():
    ...
    model = ResNet18MTLLetterVowelMakhraj().to(DEVICE)
    ...
    summary = {
        "model": "mtl_resnet18_letter_vowel_makhraj",
        "seed": SEED,
        "batch_size": BATCH_SIZE,
    ...
    print(f"Val Makhraj Acc:      {val_metrics['acc_makhraj']:.4f}")
