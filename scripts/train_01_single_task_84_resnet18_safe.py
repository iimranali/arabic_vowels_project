...
RUN_DIR = PROJECT_ROOT / "runs_torch" / "safe_single_task_resnet18_84"
...
class MelFeatureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # x expected shape: [n_mels, time]
        if x.ndim != 2:
            raise ValueError(f"Expected 2D Mel feature, got shape {x.shape} for {feature_path}")
...
class ResNet18SingleTask84(nn.Module):
    def __init__(self, num_classes=84):
        super().__init__()

        self.backbone = resnet18(weights=None)

        # Modify first conv layer for single-channel log-Mel input
...
def plot_training_curves(history_df: pd.DataFrame):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Single-Task ResNet18 84-Class Loss")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title("Single-Task ResNet18 84-Class Accuracy")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def save_reports(y_true, y_pred, split_name: str):
    ...
    report_txt_path = REPORT_DIR / f"{split_name}_84_classification_report.txt"
    report_json_path = REPORT_DIR / f"{split_name}_84_classification_report.json"
    ...
    cm_path = REPORT_DIR / f"{split_name}_84_confusion_matrix.csv"
...
def main():
    ...
    print("\nUnique original IDs")
    print("-------------------")
    print("Train:", train_df["original_id"].nunique())
    print("Val:", val_df["original_id"].nunique())
    print("Test:", test_df["original_id"].nunique())

    print("\nClass coverage")
    print("--------------")
    print("Train classes:", train_df["class_id"].nunique())
    print("Val classes:", val_df["class_id"].nunique())
    print("Test classes:", test_df["class_id"].nunique())
    ...
    model = ResNet18SingleTask84(num_classes=NUM_CLASSES).to(DEVICE)
