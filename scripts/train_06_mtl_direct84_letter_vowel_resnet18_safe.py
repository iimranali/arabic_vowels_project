...
RUN_DIR = PROJECT_ROOT / "runs_torch" / "safe_mtl_resnet18_direct84_letter_vowel"
...
class ResNet18MTLDirect84LetterVowel(nn.Module):
    def __init__(self):
        super().__init__()
...
def compute_batch_predictions(outputs):
    ...
    return direct84_pred, letter_pred, vowel_pred, derived84_pred
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        direct84_pred, letter_pred, vowel_pred, derived84_pred = (
...
def plot_training_curves(history_df: pd.DataFrame):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("MTL ResNet18 Direct84+Letter+Vowel Loss")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
def main():
    ...
    model = ResNet18MTLDirect84LetterVowel().to(DEVICE)
    ...
    summary = {
        "model": "mtl_resnet18_direct84_letter_vowel",
        "seed": SEED,
        "batch_size": BATCH_SIZE,
